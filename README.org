...
~wal-g~ has a small catch that makes it little in-convenient to run base-backups from the host: https://github.com/wal-g/wal-g/issues/1782
#+end_quote
**** Command reference
...
|---------------------------------+-------------+------------------------------------------------------+------------------------------------------------------------+-----------------------------------------------------------------|
| Warm Standby                    | No          | ~standby.signal~                                       | primary & standby operate in continuous archival/mode      |                                                                 |
| Hot Standby                     | No          | ~standby.signal~ + ~hot_standby=on~                      | Same as warm stanby but readable while WAL(s) being loaded |                                                                 |
|---------------------------------+-------------+------------------------------------------------------+------------------------------------------------------------+-----------------------------------------------------------------|
| Warm Standby                    | Yes         | No                                                   | This is no backup, just replicas                           |                                                                 |
| Hot Standby                     | Yes         | No                                                   | This is no backup, just replicas                           |                                                                 |
|---------------------------------+-------------+------------------------------------------------------+------------------------------------------------------------+-----------------------------------------------------------------|
| Warm Standby                    | Yes         | Yes                                                  | This is ideal                                              |                                                                 |
| Hot Standby                     | Yes         | Yes                                                  | This is even more ideal imo                                |                                                                 |
|---------------------------------+-------------+------------------------------------------------------+------------------------------------------------------------+-----------------------------------------------------------------|
| Crash Recovery (Normal)         | No          | Yes, use ~recovery.signal~                             | This is the most basic scenario                            | Do we really need to disable ~archive_mode~ here during recovery? |
| Crash Recovery (w Standby)      | No          | Yes, use ~recovery.signal~ and use ~standby.signal~ both | In this scenario, stanby takes precedence                  |                                                                 |
...
****** TODO Make changes to ~postgresql.conf~ for standby
...
+recovery_target_action = 'pause'	# 'pause', 'promote', 'shutdown'
  #+end_src
- Add ~standby.signal~
**** TODO Standby Servers (Sync) (ONLY via Streaming Replication)
#+begin_quote
...
***** TODO via WAL Archival (PITR)
I've not done this yet, will update when I get an opportunity to play around with it. But this is extremely similar to standby servers except the following gotchas:
- WRITE THEM DOWN
...
#+end_src
**** TODO Using ~dockerTools.fromImage~ (official docker image)
...
*** To explore later
- Streaming replication
- Compression of WAL logs, esp when setting archive_timeout before WAL hits 16MB wal segment size
