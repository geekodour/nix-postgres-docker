...
|             | ~backup-push~   | create a full ~$PGDATA~ backup                                                | periodic(auto)                                      |
|             |               | ~backup-push~ will decide on it's own when to do delta or full                |                                                     |
|             | ~delete~        | removes unnecessary WAL(s), leaving only stuff that can be used for restore | periodic(auto)                                      |
...
|             | ~wal-fetch~     | fetches WALs as part of ~restore_command~                                     | never run manually, but only run during restoration |
|-------------+---------------+-----------------------------------------------------------------------------+-----------------------------------------------------|
...
#+end_src
:(
*** Everything works ~archive_comman~ fails
- wa-g
- normal cp
- /bin/true
- archive_command = 'echo test > /tmp/archive_test.txt'
- final boss strace
  - before this see if official image is working
  - sudo strace -f -p $(head -1 /var/lib/postgresql/data/postmaster.pid) -s 1024 -o /tmp/postgres_strace.log
  - sudo strace -f -p 1049463 -s 1024 -o /tmp/postgres_strace.log
