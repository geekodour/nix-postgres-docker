...
*** dockerTools
**** Use of gosu
- The official image makes use of gosu in the runtime entry point
  - It does many things which might need root access before the main prostgres process starts
  - For that reason using gosu is more pref. here than using User or USER but you do still need to create the postgres user.
*** Setting up ~wal-g~
...
Update: writing another blogpost
**** gosu
*** Previous attempts
